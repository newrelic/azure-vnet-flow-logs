...
- **`Local Authentication`** (default): the function uses shared-key connection strings (`EVENTHUB_CONSUMER_CONNECTION`, `SOURCE_STORAGE_CONNECTION`, `CURSOR_STORAGE_CONNECTION`). Existing deployments are unaffected.
- **`Managed Identity`**: no secrets are stored. The function authenticates with its system-assigned identity and the template grants the required RBAC roles:
...
| `AUTHENTICATION_MODE` | `Local Authentication` or `Managed Identity` | `Local Authentication` |
